(() => {
  ...
  const enhanceImages = () => {
    document.querySelectorAll(".article-content img").forEach((image) => {
      if (image.hasAttribute("data-concept-animation")) return;
  ...
  const enhanceConceptAnimations = () => {
    document.querySelectorAll(".article-content img[data-concept-animation]").forEach((image) => {
      const poster = imageUrl(image);
      let animation;
      try {
        animation = new URL(image.dataset.conceptAnimation, document.baseURI);
        if (animation.origin !== location.origin || !animation.pathname.startsWith("/assets/uploads/")) return;
      } catch (_) { return; }
      if (!poster) return;
      ...
      const startAnimation = () => {
        ...
        if (observer) observer.disconnect();
        image.onload = () => {
          timer = setTimeout(showPoster, duration * 1000);
        };
        image.onerror = showPoster;
        image.src = animation.href;
      };
      // Wait until the reader reaches the diagram so the opening strokes are visible.
      if (typeof IntersectionObserver === "function") {
        observer = new IntersectionObserver((entries) => {
          if (entries.some((entry) => entry.isIntersecting && entry.intersectionRatio >= 0.25)) startAnimation();
        }, { threshold: 0.25 });
  ...
  const enhanceMedia = () => {
    enhanceImages();
    enhanceConceptAnimations();
  };
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", enhanceMedia, { once: true });
  else enhanceMedia();
})();
